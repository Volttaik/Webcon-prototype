...
router.post("/projects", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [p] = await db.insert(projectsTable).values({
      userId: req.userId!, title, agentId: agentId ?? undefined, subject, type, dueDate: dueDate ? new Date(dueDate).toISOString() : undefined,
    }).returning();
...
router.post("/projects/:id/tasks", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (!title) { res.status(400).json({ error: "title required" }); return; }
    const [task] = await db.insert(projectTasksTable).values({ projectId, title, dueDate: dueDate ? new Date(dueDate).toISOString() : undefined }).returning();
